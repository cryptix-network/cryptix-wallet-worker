...
import {Wallet, EventTargetImpl, helper, cryptixcore, CONFIRMATION_COUNT, COINBASE_CFM_COUNT} from '@cryptix/wallet';
const {HDPrivateKey} = cryptixcore;
...
export const initCryptixFramework = (opt:{workerPath?:string}={})=>{
	return new Promise<void>((resolve, reject)=>{
		helper.dpc(2000, ()=>{
			...
			if(IS_NODE_CLI){
				...
				let {
					workerPath="/node_modules/@cryptix/wallet-worker/worker.js"
				} = opt
				url = new URL(workerPath, baseURL);
			}
			workerLog.info("initCryptixFramework", url, baseURL)
...
class WalletWrapper extends EventTargetImpl{
	...
	initWorker(){
		if(!worker)
			throw new Error("Please init cryptix framework using 'await initCryptixFramework();'.")
		this.worker = worker;
